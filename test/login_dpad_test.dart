...
void main() {
  ...
  bool usernameHasFocus(WidgetTester tester) => tester
  ...
  testWidgets('right at password end reaches reveal, left returns to field', (
    tester,
  ) async {
    ...
    await arrow(tester, LogicalKeyboardKey.select);
    expect(
  ...
  testWidgets(
    'the screen opens with the first field focused, no pointer needed',
    (tester) async {
      await pumpLogin(tester);
      expect(
        usernameHasFocus(tester),
        isTrue,
  ...
  testWidgets('OK on Sign In starts a sign in attempt', (tester) async {
    ...
    expect(
      appState.busy || appState.error != null,
      isTrue,
